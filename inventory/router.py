...
@router.get("/categories/", response_model=PaginatedResponse[schemas.Category], tags=["categories"])
...
@router.delete("/categories/{category_id}", response_model=schemas.Category, tags=["categories"])
...
@router.get("/items/{item_id}", response_model=schemas.ItemRead, tags=["items"])
def read_item(item_id: int, db: Session = Depends(get_db)):
    return crud.get_item_by_id(db=db, item_id=item_id)


@router.get("/items/", response_model=PaginatedResponse[schemas.ItemRead], tags=["items"])
...
@router.put("/items/{item_id}", response_model=schemas.ItemRead, tags=["items"])
...
@router.delete("/items/{item_id}", response_model=schemas.ItemRead, tags=["items"])
def remove_item(item_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_item = crud.delete_item(db=db, item_id=item_id)
    return db_item


@router.post("/inventory/add/{item_id}", tags=["inventory"])
